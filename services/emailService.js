...
import { checkEmailHealth, sendEmail } from '../config/nodemailer.js';
...
class EmailService {
  constructor() {
    this.fromAddress = process.env.EMAIL_USER || process.env.SMTP_USER || 'noreply@buildestate.com';
  }
  ...
  async getHealthStatus() {
    try {
      const smtpHealth = await checkEmailHealth();
...
export const sendAppointmentScheduled = emailService.sendAppointmentScheduled.bind(emailService);
export const sendAppointmentStatusUpdate = emailService.sendAppointmentStatusUpdate.bind(emailService);
export const sendWelcomeEmail = emailService.sendWelcomeEmail.bind(emailService);
export const sendEmailVerification = emailService.sendEmailVerification.bind(emailService);
export const sendPasswordResetEmail = emailService.sendPasswordResetEmail.bind(emailService);
export const sendNewsletterWelcome = emailService.sendNewsletterWelcome.bind(emailService);
export const sendListingApproved = emailService.sendListingApproved.bind(emailService);
export const sendListingRejected = emailService.sendListingRejected.bind(emailService);
export const sendAppointmentNotificationToAdmin = emailService.sendAppointmentNotificationToAdmin.bind(emailService);
export const sendBulkEmails = emailService.sendBulkEmails.bind(emailService);
export const sendEmailSafely = emailService.sendEmailSafely.bind(emailService);
export const sendUserSuspended = emailService.sendUserSuspended.bind(emailService);
export const sendUserBanned = emailService.sendUserBanned.bind(emailService);
export const sendUserReactivated = emailService.sendUserReactivated.bind(emailService);
